...
def create_sql_tools(db_path: Path):
    ...
    def check_sql(sql_query: str) -> str:
        ...
        """
        llm = LLM(api_key=os.getenv("OPENAI_API_KEY"), model=os.getenv("OPENAI_MODEL_NAME"))
...
class PFFAnalyzer:
    ...
    def create_team_crew(self, team_name):
        """
        Creates a crew for a specific team to analyze their data.
        """
        ...
        report_writer = Agent(
            role="Report Writer",
            goal=f"Create clear and concise reports summarizing the insights and findings for {team_name}. The report should include actionable recommendations, clear summaries, and use markdown for structuring the output.",
        ...
        analyze_data_task = Task(
            description=dedent(f"""Use the SQL query provided by the SQL developer to get the data needed to analyze the {team_name}'s performance. The analysis should prioritize overall team grades, but provide insights into individual players. 
        ...
        return Crew(
            ...
            tasks=[extract_data_task, analyze_data_task, scouting_report_task, generate_report_task],
    ...
    def create_sqlite_db_and_load(self, team_name):
        """Creates a SQLite database and loads data from all of that team's csv files"""
    ...
    def run_team(self, team_name):
        """
        Train the crew for a specific team.
        """
        print(f"Starting analysis for {team_name}...")
        self.create_sqlite_db_and_load(team_name)
        crew = self.create_team_crew(team_name)
    ...
    def train_crew(self, n_iterations, inputs, filename):
        """
        Train the crew with the specified number of iterations and inputs.
        """
        try:
