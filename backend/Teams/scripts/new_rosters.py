...
def get_player_stats(player_name, current_team, global_stats, duplicate_names):
    ...
    if player_name in duplicate_names:
        return global_stats[(global_stats['Player'] == player_name) & 
                          (global_stats['Team'] == current_team)]

    # Get all entries for player
    player_entries = global_stats[global_stats['Player'] == player_name]
    ...
    if 'Snaps' in player_entries.columns:
        ...
        # If no snaps column, get most recent entry
...
def process_team_roster(team_folder, global_data, duplicate_names):
    ...
    # Save results
    if defense_players:
        pd.DataFrame(defense_players).to_csv(os.path.join(team_folder, 'defense_2023_2.csv'), index=False)
    if offense_players:
        pd.DataFrame(offense_players).to_csv(os.path.join(team_folder, 'offense_2023_2.csv'), index=False)
    if special_teams_players:
        pd.DataFrame(special_teams_players).to_csv(os.path.join(team_folder, 'special_teams_2023_2.csv'), index=False)
